    ...
    if (username && password) {
        ...
        if (customer) {
            if (customer.active === 1) {
                ...
                res.json({ success: true, message: 'Authentication successful', token, customer: sanitizeCustomer(customer) });
            } else {
...
router.put('/customers/:id', JwtUtil.checkToken, async function (req, res) {
    ...
    const phone = req.body.phone;
    const email = req.body.email;
    const customer = { _id, username, name, phone, email };
    ...
    res.json(sanitizeCustomer(result));
});
...
// Thêm API lấy đơn hàng (Cần thiết cho bước tiếp theo của Lab 7: My Orders)
