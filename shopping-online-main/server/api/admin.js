...
router.post('/products', JwtUtil.checkToken, async function (req, res) {
  const { name, price, category: cid, image, sizes = [], sizeGuide = '' } = req.body;
  const now = new Date().getTime();
  const category = await CategoryDAO.selectByID(cid);
  const product = { name, price, image, cdate: now, category, sizes, sizeGuide };
  const result = await ProductDAO.insert(product);
  res.json(result);
});

router.put('/products', JwtUtil.checkToken, async function (req, res) {
  const { id: _id, name, price, category: cid, image, sizes = [], sizeGuide = '' } = req.body;
  const now = new Date().getTime();
  const category = await CategoryDAO.selectByID(cid);
  const product = { _id, name, price, image, cdate: now, category, sizes, sizeGuide };
